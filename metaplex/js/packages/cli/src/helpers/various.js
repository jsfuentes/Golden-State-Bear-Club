...
exports.parseUses =
  exports.getCluster =
  exports.getPriceWithMantissa =
  exports.getMetadata =
  exports.generateRandoms =
  exports.chunks =
  ...
  exports.sleep =
  exports.getUnixTs =
  exports.generateRandomSet =
  exports.assertValidBreakdown =
  exports.shuffle =
  exports.readJsonFile =
  exports.getCandyMachineV2Config =
    void 0;
...
console.log('JORGE', fs_1);
console.log('JORGE2', fs_1.promises);
// console.log('JORGE3', fs_1['default']);
    ...
    return __generator(this, function (_c) {
      switch (_c.label) {
        case 0:
          ...
          configString = fs_1.readFileSync(configPath);
          config = JSON.parse(configString);
          (storage = config.storage),
            (ipfsInfuraProjectId = config.ipfsInfuraProjectId),
            (number = config.number),
            (ipfsInfuraSecret = config.ipfsInfuraSecret),
            (awsS3Bucket = config.awsS3Bucket),
            (noRetainAuthority = config.noRetainAuthority),
            (noMutable = config.noMutable),
            (batchSize = config.batchSize),
            (price = config.price),
            (splToken = config.splToken),
            (splTokenAccount = config.splTokenAccount),
            (solTreasuryAccount = config.solTreasuryAccount),
        ...
        case 1:
          ...
          return [
            4 /*yield*/,
            (0, accounts_1.getAtaForMint)(
              new anchor_1.web3.PublicKey(splToken),
        ...
        case 5:
          ...
          splTokenAccountKey = new anchor_1.web3.PublicKey(
            splTokenAccountFigured,
          );
        ...
        case 7:
          ...
          if (!tokenAccount.mint.equals(splTokenKey)) {
            throw new Error(
              "The spl-token-account's mint ("
                .concat(
                  tokenAccount.mint.toString(),
                  ') does not match specified spl-token ',
                )
                .concat(splTokenKey.toString()),
            );
          }
          wallet = new anchor_1.web3.PublicKey(splTokenAccountKey);
          parsedPrice = price * Math.pow(10, mintInfo.decimals);
          if (
            whitelistMintSettings === null || whitelistMintSettings === void 0
              ? void 0
              : whitelistMintSettings.discountPrice
          ) {
            whitelistMintSettings.discountPrice *= Math.pow(
              10,
              mintInfo.decimals,
            );
          }
        ...
        case 9:
          if (whitelistMintSettings) {
            whitelistMintSettings.mint = new anchor_1.web3.PublicKey(
            ...
            if (
              ...
              whitelistMintSettings.discountPrice = new anchor_1.BN(
                whitelistMintSettings.discountPrice,
              );
            }
          }
          if (endSettings) {
            if (endSettings.endSettingType.date) {
              endSettings.number = new anchor_1.BN(
                parseDate(endSettings.value),
              );
          ...
          if (gatekeeper) {
            gatekeeper.gatekeeperNetwork = new anchor_1.web3.PublicKey(
          ...
          return [
            2 /*return*/,
            {
              ...
              goLiveDate: goLiveDate
                ? new anchor_1.BN(parseDate(goLiveDate))
                : null,
              uuid: uuid,
...
var assertValidBreakdown = function (breakdown) {
  var total = Object.values(breakdown).reduce(function (sum, el) {
    return (sum += el);
  }, 0);
...
function parsePrice(price, mantissa) {
  if (mantissa === void 0) {
    mantissa = web3_js_1.LAMPORTS_PER_SOL;
  }
      ...
      switch (_a.label) {
        case 0:
          return [
            4 /*yield*/,
            Promise.all(
              chunks(keys, 99).map(function (chunk) {
          ...
          array = result
            .map(function (a) {
              ...
              return a.array.map(function (acc) {
                ...
                var obj = __assign(__assign({}, rest), {
...
var getMetadata = function (
  ...
  if (seller_fee_basis_points === void 0) {
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          ...
          return [
            4 /*yield*/,
            connection._rpcRequest('getMultipleAccounts', args),
          ];
        case 1:
          unsafeRes = _a.sent();
          if (unsafeRes.error) {
...
var getPriceWithMantissa = function (
  price,
  mint,
  walletKeyPair,
  anchorProgram,
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          token = new spl_token_1.Token(
            anchorProgram.provider.connection,
            new anchor_1.web3.PublicKey(mint),
            spl_token_1.TOKEN_PROGRAM_ID,
            walletKeyPair,
          );
          return [4 /*yield*/, token.getMintInfo()];
...
function getCluster(name) {
  for (
    var _i = 0, CLUSTERS_1 = constants_1.CLUSTERS;
    _i < CLUSTERS_1.length;
    _i++
  ) {
...
function parseUses(useMethod, total) {
  if (!!useMethod && !!total) {
    ...
    return new mpl_token_metadata_1.Uses({
      useMethod: realUseMethod,
      total: total,
      remaining: total,
    });
  }
